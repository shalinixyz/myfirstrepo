...
public class Bank_details extends Variables implements Account1 
{
		...
		try{
			for (int i = 0; i < cust.length;i++) 
			{
				...
				if(temp_accnum==cust[i].acc_num)
				{
					...
					if(!(ismultiplesofHundred(temp_amount)))
					{
						System.out.println("please enter amount in multiples of 100");
						System.out.println("Try again");
						temp_amount=(int) sc.nextDouble();
		...
		try
		{
			...
			for (int i = 0; i < cust.length; i++) 
			{
				if (cust[i].acc_num==temp_accnum) 
				{
					...
					if(!(canwithdraw(cust[i].balance, temp_amount)))
					{
					...
					{
						cust[i].balance =withdraw(cust[i].balance ,temp_amount);
	...
	int withdraw(int actual_amount,int withdraw_amount)
	{
		return actual_amount-withdraw_amount;
	}


	int Simpleinterest(int years,int rate,int balance)
	{
		int si;
		si=(int) ((balance*years*rate)/100); 
		return si;

	}

	boolean ismultiplesofHundred(int amount)
	{
	...
	boolean canwithdraw(int actual_amount,int withdraw_amount)
	{
		if((withdraw_amount<actual_amount)&& (withdraw_amount>500))
			return true;
		return false;
